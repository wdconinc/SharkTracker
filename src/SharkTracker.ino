...
// Based on example of Wake On Move with the AssetTracker and the Electron
//
...
void setup() {
	...
	ubidots.setDatasourceName("AssetTracker");
}
...
void loop() {
	...
	case PUBLISH_STATE:
		if (Particle.connected()) {
			// Location as ubidots context
			char location[25];
			snprintf(location, sizeof(location), "lat=%f$lng=%f",
					gps.location.lat(), gps.location.lng()); 

			// Publish bubble data packet to Ubidots
			ubidots.setDatasourceTag("Location");
			ubidots.add("awake",awake,location);
			ubidots.add("cell_voltage",batteryMonitor.getVCell(),location);
			ubidots.add("state_of_charge",batteryMonitor.getSoC(),location);
			ubidots.add("lat",gps.location.lat(),location);
			ubidots.add("lng",gps.location.lng(),location);
			ubidots.sendAll();
